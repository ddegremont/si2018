...
<body>
   <div class="content">
      ...
      <h1>Top 100 des
         <select name="" id="">
            <option class="options-select" value="Hôtel" selected>Hôtel</option>
            <option class="options-select" value="Restaurant">Restaurant</option>
            <option class="options-select" value="Spa">Spa</option>
          </select>
          <img class="triangle" src="/triangle.734aea57.svg" alt="triangle"></h1>
      <section class="fiches">
          <?php while ($counter < $rowLengthTemp) :?>
         <div class="fiches-full" data-category="<?=$row[$counter]['category']?>">
            <div class="fiches-number">0<?=$row[$counter]['ranking']?></div>
            <div class="fiches-container">
               <div class="fiches-picture"></div>
               <div class="fiches-description">
                  <p class="fiches-title"><?=$row[$counter]['name']?></p><br>
                  <p class="fiches-localisation"><?=$row[$counter]['localisation']?></p><br>
               </div>
            </div>
         </div>
              <?php
          ...
          endwhile;
          ?>
         <button class="fiches-addinput" type="button" name="button">Voir plus</button>
      </section>
  ...
  <footer class="footer">
   ...
   var fichesFull = document.querySelectorAll('.fiches-full');
   var select = document.querySelector('select');

   var toFilter = (target) => {
       for (var i = 0; i < fichesFull.length; i++) {
           fichesFull[i].style.display = "none";
           if (fichesFull[i].getAttribute("data-category") === target) {
               fichesFull[i].style.display = "";
           }
       }
   }

   toFilter(select.value);
