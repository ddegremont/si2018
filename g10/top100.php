...
<body>
  <div class="header no-background">
    ...
    <section class="title">
      L’actualité de nos meilleures adresses et bons plans maintenant à votre portée Les TOP 100
    </section>
    <section class="blocks-container">
      <div class="second-blocks-container">
          <div class="block-nb1">
              <p class="title-block-nb1">Restaurants</p>
          <?php
          ...
          $stmt->bindValue(':category', 'Restaurants', PDO::PARAM_STR);
          $stmt->execute();
          ...
          <?php endwhile;?>
              <a class="lasuite" href="toprestaurant.php">La suite ...</a>
          </div>
        ...
        <div class="block-nb2">
          <p class="title-block-nb1">Hôtels</p>
            ...
            while(false !== $row = $stmt->fetch(PDO::FETCH_ASSOC)):
                ...
                <p class="du-text"><span class="number-bold"><?=$row['top']?>. </span><?=$row2['h1']?></p>
            <?php endwhile;?>
          <a class="lasuite" href="tophotel.php">La suite ...</a>
        </div>
        <div class="block-nb3">
          <p class="title-block-nb1">Spa & Thalasso</p>
            <?php
            require_once "includes/connection.php";
            $sql = "SELECT
                `idtop`,
                ...
                ";
            $stmt = $pdo->prepare($sql);
            $stmt->bindValue(':category', 'SPAs et Thalassos', PDO::PARAM_STR);
            $stmt->execute();
            while(false !== $row = $stmt->fetch(PDO::FETCH_ASSOC)):
                $sql = "SELECT
                      `h1`
                ...
                <p class="du-text"><span class="number-bold"><?=$row['top']?>. </span><?=$row2['h1']?></p>
            <?php endwhile;?>
          <a class="lasuite" href="topspa.php">La suite ...</a>
        </div>
      </div>
    </section>
